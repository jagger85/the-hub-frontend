export const dataService = {
    ...
    getPortfolioWallets: function (name) {
        return JSON.parse(localStorage.getItem('portfolios')).find(
            (x) => x.name == name
        ).wallets
    },
    ...
    removePortfolio: function (portfolio) {
        ...
        if (filteredPortfolios.length == portfolios.length) {
            //Check if the portfolio existed
    ...
    addWallet: function (portfolioName, alias, wallet) {
        ...
        if (
            portfolios
                .find((x) => (x.name = portfolioName))
                .wallets.every((x) => x.alias != alias)
        ) {
            portfolios
                .find((x) => (x.name = portfolioName))
                .wallets.push({ alias: alias, address: wallet })
        }
        localStorage.setItem('portfolios', JSON.stringify(portfolios))
    },
    removeWallet: function (portfolioName, alias) {
        let portfolios = JSON.parse(localStorage.getItem('portfolios'))
        portfolios.find((x) => x.name == portfolioName).wallets = portfolios
            .find((x) => x.name == portfolioName)
            .wallets.filter((x) => x.alias != alias)
        localStorage.setItem('portfolios', JSON.stringify(portfolios))
    },
