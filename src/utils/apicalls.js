...
export const apiCalls = {
  ...
  setNetwork(network) {
    ...
    network == 'mainnet'
      ? ((this.endpoints = mainnetEndpoints), (this.endpoint = mainnetEndpoints[0]))
      : ((this.endpoints = testnetEndpoints), (this.endpoint = testnetEndpoints[0]));
  },
  ...
  getWalletTransactions: async function (params) {
    ...
    return axios.get(query).then((res) => res.data.transactions);
  },
  ...
  getWalletUniqs: async function (wallet) {
    return wallet
      ? axios
          .post(`${this.endpoint}/v1/chain/get_table_rows`, {
            code: 'eosio.nft.ft',
            table: 'token.a',
            json: true,
            scope: `${wallet}`,
  ...
  getUniqImgUrl: async function (id) {
    const imgUrl = '';
    return axios.post(`${this.endpoint}/v1/chain/get_table_rows`, {
      ...
      limit: 1,
      json: true,
    }).then(res => res.data.rows[0].meta_uris[0])
  },
