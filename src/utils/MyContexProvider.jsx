...
const MyContextProvider = ({ children }) => {
  ...
  const init = async () => {
    try {
      ...
      dispatch({type: actionTypes.UPDATE_PORTFOLIOS,portfolios: portfolios}) //UPDATE PORTFOLIOS TO STATE
      if (portfolios.length == 0){                            // IF THERE IS NO PORTFOLIOS NOTHING TO DO
      ...
      if (preferredPortfolio != '') {                                       // IF SO
        console.log(portfolios)
        console.log(preferredPortfolio)
        dispatch({ type: actionTypes.SET_SELECTED_PORTFOLIO, selectedPortfolio: preferredPortfolio }); // SET THE SELECTED PORTFOLIO TO SHOW
        dispatch({ type: actionTypes.SET_PREFERRED_PORTFOLIO, preferredPortfolio: preferredPortfolio });// SET THE PREFERRED PORTFOLIO
        dispatch({type: actionTypes.LOADED})
        return 'ok';

      } else {                        // IF THERE IS NOT A PREFERRED PORTFOLIO THEN SHOW THE FIRST ONE
        dispatch({ type: actionTypes.SET_SELECTED_PORTFOLIO, selectedPortfolio: portfolios[0]})
        console.log(portfolios)
        dispatch({ type: actionTypes.LOADED})
        return 'ok';
      }
  ...
  return <MyContext.Provider value={{ state, dispatch, init, getPortfolio }}>{children}</MyContext.Provider>;
};
