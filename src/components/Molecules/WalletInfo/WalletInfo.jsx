  ...
  return info.length != 0 ? (
    <Grid container sx={styles.container}>
      <Paper sx={styles.paper}>
        <Grid item xs={12}>
          <Typography variant='h5medium' color={colors.background[200]}>
            Net worth
          </Typography>
          <Divider />
        </Grid>
        <Grid item sx={styles.balances} key={uuid()} xs={5}>
          {info.map((wallet) => {
            return (
              <Typography variant='h7light' color={colors.background[200]} key={uuid()}>
                Balance: {parseInt(wallet.core_liquid_balance).toFixed()}
              </Typography>
            );
          })}
        </Grid>
